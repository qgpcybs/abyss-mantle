...
library CookSpecs {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "", name: "CookSpecs", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x74620000000000000000000000000000436f6f6b537065637300000000000000);
   ...
   */
  function get(bytes16 outputType) internal view returns (CookSpecsData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(outputType);
   ...
   * @notice Get the full data.
   */
  function _get(bytes16 outputType) internal view returns (CookSpecsData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(outputType);
   ...
   */
  function set(bytes16 outputType, CookSpecsData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes16 outputType, CookSpecsData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.buildingType, _table.timeCost);
  ...
  ) internal pure returns (CookSpecsData memory _table) {
